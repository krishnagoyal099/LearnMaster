...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/youtube/search', async (req, res) => {
    try {
      ...
      const enhancedQuery = timePreference === 'quick' 
      ...
      const videos = data.items.map((item: any, index: number) => {
        ...
        const irrelevantKeywords = ['music', 'song', 'funny', 'meme', 'reaction', 'unboxing', 'vlog'];
        ...
        if (title.includes('advanced') || title.includes('expert') || title.includes('master') || totalMinutes > 180) {
          difficulty = 'Advanced';
        } else if (title.includes('intermediate') || title.includes('complete') || title.includes('full course') || totalMinutes > 45) {
          difficulty = 'Intermediate';
        } else if (title.includes('beginner') || title.includes('crash course') || title.includes('basics') || title.includes('intro')) {
          difficulty = 'Beginner';
        }
...
Focus on practical, well-known books that are actually available and useful for learning ${subject}.`;
      ...
      const books = await Promise.all(bookRecommendations.map(async (book: any, index: number) => {
        let pdfUrl = `https://www.google.com/search?q=${encodeURIComponent(book.title + ' ' + book.author + ' filetype:pdf')}`;
        ...
        try {
          ...
          if (process.env.GOOGLE_SEARCH_API_KEY && process.env.GOOGLE_SEARCH_ENGINE_ID) {
            ...
            if (searchResponse.ok) {
              ...
              const pdfResult = searchData.items?.find((item: any) => 
      ...
      res.status(500).json({ error: 'Failed to fetch AI book recommendations' });
    }
  });
  ...
  app.get('/api/leaderboard/breaks', async (req, res) => {
    try {
      ...
      const leaderboard = Object.values(userStats).map((user: any) => ({
        ...
        completionRate: user.totalBreaks > 0 ? (user.completedBreaks / user.totalBreaks * 100).toFixed(1) : 0,
        rank: 0
